...
class StatDeckApp {
    ...
    loadConfig(layout) {
        ...
        // --- Parse pages from layout ---
        this.pages = this.parsePagesFromLayout(layout);
        this.currentPageIndex = 0;
        
        console.log(`Loaded ${this.pages.length} page(s)`);
        
        // --- Apply theme (shared across all pages) ---
        ...
        if (this.themeData && this.themeData.colors) {
            ...
            console.log(`Theme applied: ${this.themeId} (${this.themeData.name || 'unnamed'})`);
     ...
     */
    get TRANSITION_EXIT_MS() { return 220; }
    get TRANSITION_ENTER_MS() { return 350; }
     ...
     * 
     * Flow:
     *   1. Add .page-exit-[direction] → exit animation plays
     *   2. After exit duration, swap tiles (renderCurrentPage)
     *   3. Add .page-enter-[direction] → enter animation plays
     *   4. After enter duration, clean up classes
     *
     * CSS drives the visuals, JS just toggles classes and rebuilds DOM.
     */
    transitionToPage(newIndex, direction) {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                console.log('Switched to page ' + (this.currentPageIndex + 1) + '/' + this.pages.length);
