/**
 * Live Preview Bridge - WITH ALL NEW STYLES
 * Real data + Line graphs, Semi-circle gauges, Sparklines, etc.
 */
...
class LivePreviewBridge {
    ...
    updateTilePreview(element, config, value, fullStats, tileId) {
        ...
        // Build history for time-series data
        if (config.type.includes('graph') || config.style === 'sparkline') {
        ...
        // Render based on tile type and style
        const type = config.type;
        const style = config.style || 'default';
        
        if (type === 'graph' || type === 'cpu_graph') {
            this.renderGraph(preview, config, value, fullStats, tileId, style);
        } else if (type === 'network_graph') {
            this.renderNetworkGraph(preview, config, fullStats, tileId, style);
        } else if (type === 'gauge') {
            this.renderGauge(preview, config, value, fullStats, style);
        } else if (type === 'text_display') {
            this.renderDisplay(preview, config, value, fullStats, style);
        } else if (type === 'button') {
    ...
    renderGraph(preview, config, value, stats, tileId, style) {
    ...
    renderLineGraph(preview, history, color, value, label) {
        ...
        const points = history.map((v, i) => {
            ...
            const y = height - (v / 100) * height * 0.8;
            return `${x},${y}`;
        });
        
        const pathData = history.length > 1 ? this.createSmoothPath(history, width, height) : '';
    ...
    renderAreaGraph(preview, history, color, value, label) {
        const width = 100;
        const height = 100;
        const pathData = this.createSmoothPath(history, width, height);
        ...
        preview.innerHTML = `
            ...
            <svg style="position:absolute;inset:0;width:100%;height:100%;padding:35px 8px 8px" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none">
                <defs>
                    <linearGradient id="grad-${Math.random()}" x1="0%" y1="0%" x2="0%" y2="100%">
                        <stop offset="0%" style="stop-color:${color};stop-opacity:0.6" />
                        <stop offset="100%" style="stop-color:${color};stop-opacity:0.1" />
                    </linearGradient>
                </defs>
                <path d="${areaPath}" fill="url(#grad-${Math.random()})" stroke="${color}" stroke-width="2" opacity="0.9"/>
            </svg>
        `;
    }
    ...
    renderNetworkGraph(preview, config, stats, tileId, style) {
    ...
    renderGauge(preview, config, value, stats, style) {
        ...
        preview.innerHTML = `
            ...
            <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;padding-top:20px">
                <svg width="120" height="80" viewBox="0 0 120 80">
                    <path d="M 10 70 A 50 50 0 0 1 110 70" fill="none" stroke="#333" stroke-width="10" stroke-linecap="round"/>
                    <path d="M 10 70 A 50 50 0 0 1 110 70" fill="none" stroke="${color}" stroke-width="10" 
                          stroke-linecap="round" stroke-dasharray="157" stroke-dashoffset="${157 * (1 - percent/100)}"
                          style="transition:stroke-dashoffset 0.3s"/>
                    <circle cx="60" cy="70" r="8" fill="${color}"/>
                    <line x1="60" y1="70" x2="60" y2="30" stroke="${color}" stroke-width="3" 
                          style="transform-origin:60px 70px;transform:rotate(${degrees}deg);transition:transform 0.3s"/>
                </svg>
                <div style="font-size:24px;font-weight:700;color:${color};line-height:1;margin-top:-10px">
                    ${displayValue}${unit}
                </div>
            </div>
        `;
    ...
    renderDisplay(preview, config, value, stats, style) {
        const color = config.style?.color || '#00ff88';
        const dataSource = config.data_source;
        const label = this.getDataLabel(dataSource);
        ...
        switch(style) {
            case 'sparkline':
                this.renderSparkline(preview, config, value, displayValue, unit, color, label);
                break;
            case 'icon':
                this.renderIconValue(preview, config, displayValue, unit, color, label);
                break;
            case 'multi':
                this.renderMultiValue(preview, config, stats, color, label);
                break;
            case 'number':
            default:
    ...
    renderSparkline(preview, config, value, displayValue, unit, color, label) {
    ...
    renderIconValue(preview, config, displayValue, unit, color, label) {
        const icons = {
            'cpu': '🖥️', 'gpu': '🎮', 'ram': '💾', 'disk': '💿', 
        ...
        if (config.data_source) {
            for (const [key, val] of Object.entries(icons)) {
                if (config.data_source.includes(key)) {
                    icon = val;
                    break;
                }
            }
    ...
    renderMultiValue(preview, config, stats, color, label) {
        // Show related stats together (e.g., CPU usage + temp, RAM used + percent)
        const ds = config.data_source;
        let primary = '', secondary = '', tertiary = '';
        
        if (ds && ds.includes('cpu')) {
