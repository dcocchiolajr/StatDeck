...
usb.on('message', (message) => {
    ...
    if (msgType === 'stats') {
        ...
        // Config App requesting current layout
        console.log('Config App requesting current layout');
        usb.send({
            type: 'layout_response',
            layout: ConfigLoader.load(),
            timestamp: Date.now()
        });
    }
    else {
...
usb.on('connected', () => {
    ...
    // Only request configuration if we don't have a valid layout
    ...
    if (!currentLayout || !currentLayout.tiles || currentLayout.tiles.length === 0) {
        ...
        console.log('Using existing layout with', currentLayout.tiles.length, 'tiles');
    }
});
